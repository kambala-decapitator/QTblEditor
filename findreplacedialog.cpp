...
void FindReplaceDialog::getFoundStrings(QList<QTableWidgetItem *> foundItems)
{
	...
	{
		QApplication::beep();
		QMessageBox::information(this, QApplication::applicationName(), tr("String \"%1\" not found").arg(_query));
		_searchFailed = true;
		return;
	}
...
void FindReplaceDialog::replaceAll()
{
	...
	for (_currentStringIterator = _foundStrings.begin(); _currentStringIterator != _foundStrings.end(); _currentStringIterator++, n++)
		replaceInCurrentString();
	QMessageBox::information(this, tr("Replace"), tr("%n occurrence(s) replaced", "", n));
